 ...
 * @see #links
 */
pv.Force.hierarchy = function(hk) {
  var d = .1, // default damping factor
      l = 20, // default rest length
      links, // links on which to apply spring forces
      kl, // per-spring normalization
      hdx = 20,  // X hierarchy distance, try to maintain this distance between hierarchy
      hdy = 20,  // Y hierarchy distance, try to maintain this distance between hierarchy
      fd = 10,
      force = {};

  if (!arguments.length) hk = .1; // default spring constant (tension)
  ...
  // hierarchy force
  force.hierarchyForce =function(x) {
    if (arguments.length) {
      hk = Number(x);
      return force;
    }
  ...
  // hierarchy distance
  force.hierarchyDistanceX =function(x) {
    if (arguments.length) {
      hdx = Number(x);
      return force;
    }
  ...
  // hierarchy distance
  force.hierarchyDistanceY =function(x) {
    if (arguments.length) {
      hdy = Number(x);
      return force;
    }
  ...
  // hierarchy distance
  force.fuzzyDistance =function(x) {
    if (arguments.length) {
      fd = Number(x);
      return force;
    }
  ...
  force.apply = function(particles)
  {
    ...
    for (var i = 0; i < links.length; i++)
    {
      var a = links[i].sourceNode,
        ...
        var diffY =  hdy - dy;
        var diffX =  hdx - dx;
        diffY = clampDiff(diffY, fd);
        diffX = clampDiff(diffX, fd);
 ...
 * E. Reingold, Software--Practice &amp; Experience, November 1991.
 */
pv.Layout.ForceWithHierarchy = function() {
  pv.Layout.Network.call(this);
...
pv.Layout.ForceWithHierarchy.prototype = pv.extend(pv.Layout.Network)
    .property("bound", Boolean)
    .property("iterations", Number)
    .property("dragConstant", Number)
    .property("chargeConstant", Number)
    ...
    .property("springDamping", Number)
    .property("springLength", Number)
    .property("hierarchyForce", Number)
    .property("hierarchyDistanceX", Number)
    .property("hierarchyDistanceY", Number)
    .property("fuzzyDistance", Number);
 ...
 *
 * @type boolean
 * @name pv.Layout.ForceWithHierarchy.prototype.bound
 */
 ...
 *
 * @type number
 * @name pv.Layout.ForceWithHierarchy.prototype.iterations
 */
 ...
 *
 * @type number
 * @name pv.Layout.ForceWithHierarchy.prototype.dragConstant
 * @see pv.Force.drag#constant
 */
 ...
 *
 * @type number
 * @name pv.Layout.ForceWithHierarchy.prototype.chargeConstant
 * @see pv.Force.charge#constant
 */
 ...
 *
 * @type number
 * @name pv.Layout.ForceWithHierarchy.prototype.chargeMinDistance
 * @see pv.Force.charge#domain
 */
 ...
 *
 * @type number
 * @name pv.Layout.ForceWithHierarchy.prototype.chargeMaxDistance
 * @see pv.Force.charge#domain
 */
 ...
 *
 * @type number
 * @name pv.Layout.ForceWithHierarchy.prototype.chargeTheta
 * @see pv.Force.charge#theta
 */
 ...
 *
 * @type number
 * @name pv.Layout.ForceWithHierarchy.prototype.springConstant
 * @see pv.Force.spring#constant
 */
 ...
 *
 * @type number
 * @name pv.Layout.ForceWithHierarchy.prototype.springDamping
 * @see pv.Force.spring#damping
 */
 ...
 *
 * @type number
 * @name pv.Layout.ForceWithHierarchy.prototype.springLength
 * @see pv.Force.spring#length
 */
 ...
 * @type pv.Layout.Force
 */
pv.Layout.ForceWithHierarchy.prototype.defaults = new pv.Layout.ForceWithHierarchy()
    .extend(pv.Layout.Network.prototype.defaults)
    .dragConstant(.1)
    .chargeConstant(-40)
    .chargeMinDistance(2)
    ...
    .springLength(20)
    .hierarchyForce(0.3)
    .hierarchyDistanceY(80)
    .hierarchyDistanceX(0)
    .fuzzyDistance(5);

/** @private Initialize the physics simulation. */
pv.Layout.ForceWithHierarchy.prototype.buildImplied = function(s) {
  ...
  sim.force(pv.Force.hierarchy(s.hierarchyForce)
      .hierarchyDistanceY(s.hierarchyDistanceY)
      .hierarchyDistanceX(s.hierarchyDistanceX)
      .fuzzyDistance(s.fuzzyDistance)
      .links(links));
  /* Position constraint (for interactive dragging). */
  sim.constraint(pv.Constraint.position());
